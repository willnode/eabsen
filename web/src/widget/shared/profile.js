    ...
    <Page className="paper" maxWidth="sm" src={`${role}/profile`}>
      {({ data }) => (
        <Form action={`${role}/profile`} redirect={submit}>
          ...
          {role === 'dosen' && <Input validator={validators.identity} name="nip" label="NIP" defaultValue={data.nip}/>}
          {role === 'mahasiswa' && <Input validator={validators.identity} name="nim" label="NIM" defaultValue={data.nim}/>}
