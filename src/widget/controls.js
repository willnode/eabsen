 ...
 */
function controlTable(data, columns) {
	const id = data.id ? data.id : 'table';
	const cls = data.class ? data.class : 'table-sm';
	// let script = [];
	let toolbar = <></>;
	delete data.id;
	delete data.class;
	...
	data.pagination || (data.pagination = 'true');
	data['sidePagination'] || (data['sidePagination'] = 'server');
	if (data.toolbar) {
		toolbar = <div id={id + 'toolbar'}>{data.toolbar}</div>;
		data.toolbar = '#' + id + 'toolbar';
		data['toolbarAlign'] || (data['toolbarAlign'] = 'right');
	}
	if (data['detailFormatter']) {
		data['detailFormatter'] = ((x) => ((a,b,c)=>renderToStaticMarkup(x(a,b,c))))(data['detailFormatter'])
		data['detailView'] = 'true';
	}
	columns = columns.map(column => {
		if (column['formatter']) {
			column['formatter'] = ((x) => ((a,b,c)=>renderToStaticMarkup(<Router history={session.history}>{x(a,b,c)}</Router>)))(column['formatter'])
		}
		return column;
	});
	return <>{toolbar}
		<BsTable id={id} className={cls} {...data} columns={columns}>
		</BsTable>
		</>
}
